    ...
    private class TXThread extends Thread {
        ...
        public void transmitFrame(DataFrame frame) throws InterruptedException {
    		...
    		if (frame != null) {
    			...
    			byte[] payload = frame.getTransmittedBytes(); //从dataframe中拿取payload
    	...
    	public void run() {
        	...
        	try {
	    		...
	    		while (true) {
					...
					//添加验证
					int receivedchecksum=getchecksum(bytePayload);
					byte headaddress=bytePayload[0];
					int sendchecksum=bytePayload[bytePayload.length-1];
					if (receivedchecksum!=sendchecksum || headaddress!=)
